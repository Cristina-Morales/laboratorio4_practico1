  ...
  axios.get(`${process.env.URL}animals${filtro}`)
    ...
    .catch((error) => {
      ...
      if (status >= 400 && status < 500) {
        res.status(400).json(prepararError('error', 'Bad Request'))
      } else {
        res.status(500).json(prepararError('error', 'Error inesperado al obtener la informacion'))
      }
    })
}
...
const getAnimalByID = (req = request, res = response) => {
  ...
  axios.get(`${process.env.URL}animals/${idAnimal}`).then((response) => {
    ...
    // Filtramos solo los campos que necesitamos: name, family, habitat, diet, description
    const { name, family, habitat, diet, description } = data
